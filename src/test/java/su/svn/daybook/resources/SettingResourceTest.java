...
import su.svn.daybook.services.SettingService;
...
class SettingResourceTest {
    ...
    static Uni<Answer> test = Uni.createFrom()
            .item(1)
            .onItem()
            .transform(i -> Answer.of(TestData.SETTING.OBJECT_0));
    ...
    void setUp() {
        ...
        Mockito.when(mock.getAll()).thenReturn(Multi.createFrom().item(Answer.of(TestData.SETTING.OBJECT_0)));
        Mockito.when(mock.add(TestData.SETTING.OBJECT_0)).thenReturn(TestData.UNI_ANSWER_API_RESPONSE_ZERO_LONG);
        Mockito.when(mock.put(TestData.SETTING.OBJECT_0)).thenReturn(TestData.UNI_ANSWER_API_RESPONSE_ZERO_LONG);
