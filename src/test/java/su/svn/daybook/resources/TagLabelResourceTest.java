...
class TagLabelResourceTest {
    ...
    static Uni<Answer> test = Uni.createFrom()
            .item(1)
            .onItem()
            .transform(i -> Answer.of(DataTest.TAG_LABEL.OBJECT_0));
    ...
    void setUp() {
        ...
        Mockito.when(mock.get(DataTest.TAG_LABEL.ID)).thenReturn(test);
        ...
        Mockito.when(mock.getAll()).thenReturn(Multi.createFrom().item(Answer.of(DataTest.TAG_LABEL.OBJECT_0)));
        Mockito.when(mock.add(DataTest.TAG_LABEL.OBJECT_0)).thenReturn(DataTest.TAG_LABEL.UNI_ANSWER_API_RESPONSE_ID);
        Mockito.when(mock.put(DataTest.TAG_LABEL.OBJECT_0)).thenReturn(DataTest.TAG_LABEL.UNI_ANSWER_API_RESPONSE_ID);
        Mockito.when(mock.delete(DataTest.TAG_LABEL.ID)).thenReturn(DataTest.TAG_LABEL.UNI_ANSWER_API_RESPONSE_ID);
    ...
    void testEndpointGet() {
        given()
                .when()
                .get("/tag/" + DataTest.TAG_LABEL.ID)
                .then()
                .statusCode(200)
                .body(CoreMatchers.startsWith(DataTest.TAG_LABEL.JSON_0));
    }
    ...
    void testEndpointGetAll() {
        given()
                ...
                .then()
                .statusCode(200)
                .body(CoreMatchers.startsWith(DataTest.TAG_LABEL.JSON_ARRAY_SINGLETON_0));
    }
    ...
    void testEndpointAdd() {
        given()
                ...
                .body(DataTest.TAG_LABEL.OBJECT_0)
                .when()
                .post("/tag")
                .then()
                .statusCode(200)
                .body(CoreMatchers.startsWith("{\"id\":\"" + DataTest.TAG_LABEL.ID));
    }
    ...
    void testEndpointPut() {
        given()
                .header(HttpHeaders.CONTENT_TYPE, MediaType.APPLICATION_JSON)
                .body(DataTest.TAG_LABEL.JSON_0)
                .when()
                .put("/tag")
                .then()
                .statusCode(200)
    ...
    void testEndpointDelete() {
        given()
                .when()
                .delete("/tag/" + DataTest.TAG_LABEL.ID)
                .then()
                .statusCode(200)
                .body(CoreMatchers.startsWith("{\"id\":\"" + DataTest.TAG_LABEL.ID));
    }
