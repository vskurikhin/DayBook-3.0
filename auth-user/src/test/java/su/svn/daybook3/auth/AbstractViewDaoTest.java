...
import su.svn.daybook3.auth.domain.dao.DaoIface;
import su.svn.daybook3.auth.domain.dao.DaoViewIface;
...
class AbstractViewDaoTest
    ...
    void checkCount2() {
        Assertions.assertDoesNotThrow(() -> Assertions.assertEquals(3, uniOptionalHelper(viewDao.count())));
    }
    ...
    void whenFindRangeFromZeroToMaxValueThenMultiWithTwoItems(int expected) {
        Assertions.assertDoesNotThrow(() -> {
            var test = multiAsListHelper(viewDao.findRange(0, Long.MAX_VALUE));
            Assertions.assertNotNull(test);
            Assertions.assertFalse(test.isEmpty());
            Assertions.assertEquals(expected, test.size());
        });
    }
